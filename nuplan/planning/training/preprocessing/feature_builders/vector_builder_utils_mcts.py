...
def get_route_lane_polylines_from_roadblock_ids(
    ...
    return (MapObjectPolylines(route_lane_polylines),LaneSegmentLaneIDs(lane_ids),)
...
def get_neighbor_vector_set_map(
    ...
    if VectorFeatureLayer.ROUTE_LANES in feature_layers:
        route_polylines, lane_ids = get_route_lane_polylines_from_roadblock_ids(map_api, point, radius, route_roadblock_ids)
        coords[VectorFeatureLayer.ROUTE_LANES.name] = route_polylines

        # extract traffic light
        for t, traffic_lights in enumerate(traffic_light_statuses_over_time):
            # lane traffic light data
            traffic_light_data_over_time[t][VectorFeatureLayer.ROUTE_LANES.name] = get_traffic_light_encoding(
                lane_ids, traffic_lights.traffic_lights)
