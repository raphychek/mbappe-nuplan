...
class MultipleAgentsTrajectories(AbstractModelFeature):
    ...
    def features_dim(self) -> int:
        ...
        """
        return self.num_frames * M.states_dim()

    @classmethod
    def collate(cls, batch: List[MultipleAgentsTrajectories]) -> MultipleAgentsTrajectories:
        """
        Implemented. See interface.
        Collates a list of features that each have batch size of 1.
        """
        return MultipleAgentsTrajectories(data=[item.data[0] for item in batch])

    def to_feature_tensor(self) -> MultipleAgentsTrajectories:
        """Implemented. See interface."""
        return MultipleAgentsTrajectories(data=[to_tensor(data) for data in self.data])

    def to_device(self, device: torch.device) -> MultipleAgentsTrajectories:
        """Implemented. See interface."""
        return MultipleAgentsTrajectories(data=[data.to(device=device) for data in self.data])

    @classmethod
    def deserialize(cls, data: Dict[str, Any]) -> MultipleAgentsTrajectories:
        """Implemented. See interface."""
        return MultipleAgentsTrajectories(data=data["data"])

    def unpack(self) -> List[MultipleAgentsTrajectories]:
        """Implemented. See interface."""
        return [MultipleAgentsTrajectories([data]) for data in self.data]
