...
@dataclass
class MultipleTrajectory(AbstractModelFeature):
    """
    ...
    def __post_init__(self) -> None:
        ...
        state_size = self.data[0].shape[-1]

        if (array_dims != 2) and (array_dims != 3) and (array_dims != 4):
    ...
    def to_device(self, device: torch.device) -> MultipleTrajectory:
        """Implemented. See interface."""
        validate_type(self.data, torch.Tensor)
        return MultipleTrajectory(data=self.data.to(device=device))

    def to_feature_tensor(self) -> MultipleTrajectory:
        """Inherited, see superclass."""
        return MultipleTrajectory(data=to_tensor(self.data))

    @classmethod
    def deserialize(cls, data: Dict[str, Any]) -> MultipleTrajectory:
        """Implemented. See interface."""
        return MultipleTrajectory(data=data["data"])

    def unpack(self) -> List[MultipleTrajectory]:
        """Implemented. See interface."""
        return [MultipleTrajectory(data[None]) for data in self.data]
    ...
    def num_dimensions(self) -> int:
        ...
        """
        return 1+len(self.data[0].shape)
    ...
    def extract_number_of_last_states(self, number_of_states: int) -> MultipleTrajectory:
        """
    ...
    def extract_trajectory_between(self, start_index: int, end_index: Optional[int]) -> MultipleTrajectory:
        """
    ...
    @classmethod
    def append_to_trajectory(cls, trajectory: MultipleTrajectory, new_state: torch.Tensor) -> MultipleTrajectory:
        """
        ...
        return MultipleTrajectory(data=torch.cat((trajectory.data, new_state.clone()), dim=1))
