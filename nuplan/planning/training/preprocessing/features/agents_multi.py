...
@dataclass
class AgentsMulti(AbstractModelFeature):
    """
    ...
    agents_future: List[FeatureDataType]
    ...
    @classmethod
    def collate(cls, batch: List[Agents]) -> AgentsMulti:
        """
        Implemented. See interface.
        Collates a list of features that each have batch size of 1.
        """
        return AgentsMulti(ego=[item.ego[0] for item in batch], agents=[item.agents[0] for item in batch], agents_future=[item.agents_future[0] for item in batch] )

    def to_feature_tensor(self) -> AgentsMulti:
        """Implemented. See interface."""
        return AgentsMulti(ego=[to_tensor(ego) for ego in self.ego], agents=[to_tensor(agents) for agents in self.agents], agents_future=[to_tensor(agents_future) for agents_future in self.agents_future])

    def to_device(self, device: torch.device) -> AgentsMulti:
        """Implemented. See interface."""
        return AgentsMulti(
            ego=[to_tensor(ego).to(device=device) for ego in self.ego],
            agents=[to_tensor(agents).to(device=device) for agents in self.agents],
            agents_future=[to_tensor(agents_future).to(device=device) for agents_future in self.agents_future],
        )

    @classmethod
    def deserialize(cls, data: Dict[str, Any]) -> AgentsMulti:
        """Implemented. See interface."""
        return Agents(ego=data["ego"], agents=data["agents"], agents_future=data["agents_future"])

    def unpack(self) -> List[AgentsMulti]:
        """Implemented. See interface."""
        return [AgentsMulti([ego], [agents], [agents_future]) for ego, agents, agents_future in zip(self.ego, self.agents, self.agents_future)]
    ...
    def ego_feature_dim(self) -> int:
        ...
        """
        return AgentsMulti.ego_state_dim() * self.num_frames
    ...
    def agents_features_dim(self) -> int:
        ...
        :return: ego feature dimension. Note, the plus one is to account for the present frame
        """
        return AgentsMulti.agents_states_dim() * self.num_frames
