...
class GenericAgentDropoutAugmentorMulti(AbstractAugmentor):
        ...
        for feature_name in features['generic_agents'].agents.keys():
            for batch_idx in range(len(features['generic_agents'].agents[feature_name])):
                ...
                if feature_name == 'VEHICLE':
                    print('ok')
                    targets['agents_trajectories'].data[batch_idx] = (
                        targets['agents_trajectories'].data[batch_idx].take(agent_indices, axis=1)
                    )
