...
class LocalSubGraphLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, invalid_mask: torch.Tensor) -> torch.Tensor:
        ...
        masked_x = x_.masked_fill(invalid_mask[..., None] > 0, float("-inf"))
        x_agg = masked_x.max(dim=1, keepdim=True).values
        # repeat it along the sequence length
        x_agg = x_agg.repeat(1, num_points, 1)
        x_ = torch.cat([x_, x_agg], dim=-1)
        x = self.linear_remap(x_)+x  # remap to a possibly different feature length
        return x
...
class MultiheadAttentionGlobalHeadMulti(nn.Module):
    """
    Copied from L5Kit's implementation `MultiheadAttentionGlobalHead`:
    https://github.com/woven-planet/l5kit/blob/master/l5kit/l5kit/planning/vectorized/global_graph.py.
    Changes:
    ...
    def forward(
        self, query: torch.Tensor, key: torch.tensor, type_embedding: torch.Tensor, mask: torch.Tensor, n_agents: int
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        """
        Forward of the module.
        :param inputs: Model inputs. [1 + N + M, batch_size, feature_dim]
        ...
        #   - value is inputs
        #print(inputs.size())
        n_items, batch_size, feats = query.size()
        # out, attns = self.encoder(inputs[:n_agents], inputs + type_embedding, inputs, mask)
        #print(query.shape, key.shape, type_embedding.shape)
        out, attns = self.encoder(query, key+type_embedding, key, mask)
        # print(mask)
        # print(out)
        # print()
        #out = out+query#[:n_agents]
        out = torch.nan_to_num(out)+query
        #print(out.shape)
        out = out.transpose(0, 1)
        #print(out.shape)
        outputs = F.relu(self.output_embed(out)+out)
        # outputs = self.final_output(outputs)
        # #print(outputs.shape)
        # outputs = outputs.view(batch_size, n_agents, self.num_timesteps, self.num_outputs)
        #print(outputs.shape)
        return outputs, attns