...
from nuplan.planning.training.preprocessing.target_builders.multiple_agents_trajectory_target_builder import MultipleAgentsTrajectoryTargetBuilder
...
class UrbanMultiModel(TorchModuleWrapper):
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            target_builders=[EgoTrajectoryTargetBuilder(target_params.future_trajectory_sampling), MultipleAgentsTrajectoryTargetBuilder(future_trajectory_sampling=target_params.future_trajectory_sampling)],
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        agent_embeddings, attns = self.global_head2(agent_embeddings, agent_embeddings, agent_types, agent_polys, n_agents)

        outputs = self.final_output(agent_embeddings)
        outputs = outputs.view(batch_size, n_agents, self.future_trajectory_sampling.num_poses, self._target_params.num_output_features // self.future_trajectory_sampling.num_poses)
        return {"trajectory": Trajectory(data=convert_predictions_to_trajectory(outputs[:, 0])), "agents_trajectories": MultipleAgentsTrajectories(outputs)}
